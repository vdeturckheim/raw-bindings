...
export function buildParseOptions(
  ...
  let flags = (clang as any).CXTranslationUnit_None || 0;

  if (options.detailedProcessing !== false) {
    flags |= ((clang as any).CXTranslationUnit_DetailedPreprocessingRecord ?? 0);
  }

  if (options.includeDocumentation !== false) {
    flags |= ((clang as any).CXTranslationUnit_IncludeBriefCommentsInCodeCompletion ?? 0);
  }
