...
function combineASTs(asts: HeaderAST[], headerPaths: string[]): HeaderAST {
  ...
  for (const ast of asts) {
    ...
    (combined.functions as any[]).push(...(ast.functions || []));
    (combined.classes as any[]).push(...(ast.classes || []));
...
export async function generateBindings(
  ...
  const parseOptions = {
    language: (options.defines?.includes('__cplusplus') ? 'c++' : 'c') as 'c' | 'c++',
  ...
  if (headerPaths.length === 1) {
    ...
    const firstHeader = headerPaths[0]!;
    combinedAST = parseHeader(firstHeader, parseOptions);
  } else {
  ...
  if (headerIncludePaths[0]) {
    (updatedOptions as any).headerIncludePath = headerIncludePaths[0];
  }
