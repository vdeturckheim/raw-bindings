...
describe('h-parser', () => {
  it('should parse a simple C header file', () => {
    ...
    if (statusEnum) {
      assert.ok(
        statusEnum.constants.some(
          (c) => c.name === 'STATUS_SUCCESS' && c.value === 0,
        ),
      );
      assert.ok(
        statusEnum.constants.some(
          (c) => c.name === 'STATUS_ERROR' && c.value === -1,
        ),
      );
      assert.ok(
        statusEnum.constants.some(
          (c) => c.name === 'STATUS_TIMEOUT' && c.value === -2,
        ),
      );
      assert.ok(
        statusEnum.constants.some(
          (c) => c.name === 'STATUS_NOT_FOUND' && c.value === -3,
        ),
      );
    }
    ...
    if (pointStruct) {
      assert.ok(
        pointStruct.fields.some((f) => f.name === 'x' && f.type === 'float'),
      );
      assert.ok(
        pointStruct.fields.some((f) => f.name === 'y' && f.type === 'float'),
      );
    }
    ...
    assert.ok(
      rectangleStruct.fields.some(
        (f) => f.name === 'area' && f.type === 'float',
      ),
    );
    ...
    const calcDistance = ast.functions.find(
      (f) => f.name === 'calculate_distance',
    );
    ...
    const initSystem = ast.functions.find(
      (f) => f.name === 'initialize_system',
    );
  ...
  it('should handle documentation extraction', () => {
    ...
    assert.ok(
      ...
      'Should have extracted documentation for some functions',
    );
    ...
    if (calcDistance?.documentation) {
      assert.ok(
        calcDistance.documentation.includes('distance between two points'),
      );
