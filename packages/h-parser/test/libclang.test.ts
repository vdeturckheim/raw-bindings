...
const LIBCLANG_HEADERS_PATH =
  '/opt/homebrew/Cellar/llvm/20.1.8/include/clang-c';
...
describe('libclang headers parsing', () => {
    ...
    for (const header of LIBCLANG_HEADERS) {
      it(`should parse ${header}`, () => {
        ...
        if (header === 'Index.h') {
          ...
          assert.ok(
            foundCount >= 2,
            `Should find at least 2 of the known functions in ${header}, found ${foundCount}`,
          );
        } else if (header === 'ExternC.h' || header === 'Platform.h') {
          // These are utility headers, might not have functions
          console.log(
            `${header} is a utility header, may not contain functions`,
          );
        } else {
  ...
  it('should parse the main Index.h header with documentation', () => {
    ...
    assert.ok(ast);
    assert.ok(
      ast.functions && ast.functions.length > 50,
      `Index.h should have many functions (found ${ast.functions.length})`,
    );

    // Check for documentation in some functions
    const functionsWithDocs =
      ast.functions?.filter((f) => f.documentation) || [];
    assert.ok(
      functionsWithDocs.length > 0,
      'Some functions should have documentation',
    );
    ...
    assert.ok(
      cursorKindEnum.constants.length > 5,
      'CXCursorKind should have many constants (found ' +
        cursorKindEnum.constants.length +
        ')',
    );

    // Check for important typedefs
    const importantTypedefs = [
      'CXIndex',
      'CXTranslationUnit',
      'CXCursor',
      'CXType',
  ...
  it('should handle complex C constructs in libclang headers', () => {
    ...
    const functionPointerTypedefs = ast.typedefs.filter(
      (t) => t.underlying.includes('(*)') || t.underlying.includes('(*)'),
    );
    assert.ok(
      functionPointerTypedefs.length > 0,
      'Should have function pointer typedefs',
    );
    ...
    const _variadicFunctions =
      ast.functions?.filter((f) =>
        f.params.some(
          (p) => p.type.spelling === '...' || p.type.spelling.includes('...'),
        ),
      ) || [];
    ...
    assert.ok(
      true,
      'Successfully parsed complex libclang headers without crashing',
    );
  });
});
